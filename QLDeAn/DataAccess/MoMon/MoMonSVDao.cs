...
namespace QLDeAn.DataAccess.MoMon
{
    class MoMonSVDao : IMoMonDao
    {
        ...
        public List<object> Load(object obj)
        {
            ...
            using (var cmd = new OracleCommand("SELECT * FROM QLDL.VIEW_MOMON_SV", sqlConnection))
            {
                cmd.CommandType = CommandType.Text; // Thay vì StoredProcedure
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var mm = new Model.MoMon
                        {
                            ...
                            HK = reader["hk"] != DBNull.Value ? (int?)Convert.ToInt32(reader["hk"]) : null,
                            NAM = reader["nam"] != DBNull.Value ? (int?)Convert.ToInt32(reader["nam"]) : null
                        };
